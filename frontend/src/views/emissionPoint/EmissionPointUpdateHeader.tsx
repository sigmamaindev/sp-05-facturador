...
interface EmissionPointUpdateHeaderProps {
  code: string;
}

export default function EmissionPointUpdateHeader({
  code,
}: EmissionPointUpdateHeaderProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:flex md:flex-row md:justify-between md:items-center items-center gap-4 pb-4">
      <div className="grid grid-cols-2 items-center gap-4 md:inline-flex w-auto">
        <h1 className="text-lg font-semibold">{`ACTUALIZAR PUNTO EMISIÓN - ${code}`}</h1>
      </div>
      <div className="inline-flex items-center gap-4 w-auto">
        <Button
          variant="outline"
          onClick={() => navigate("/puntos-emision")}
